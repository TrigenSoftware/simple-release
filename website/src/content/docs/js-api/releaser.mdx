...
| `preamble` | Custom markdown inserted into the changelog after the version header, before the generated sections. In a monorepo it applies to every package; a per-package `byProject[name].preamble` overrides it for that package. |
